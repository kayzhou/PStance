...
def run_classifier():
    ...
    parser.add_argument("--input_target", type=str, default="trump")
    parser.add_argument(
        "--model_select", 
        type=str, 
        default="Bert", 
    ...
    for target_index in range(len(target_word_pair)):
        ...
        for seed in random_seeds:    
            ...
            x_train, y_train, x_train_target = pp.clean_all(
                filename1, normalization_dict
            )
            x_val, y_val, x_val_target = pp.clean_all(
                filename2, normalization_dict
            )
            x_test, y_test, x_test_target = pp.clean_all(
                filename3, normalization_dict
            )
            ...
            # prepare for model
            x_train_all, x_val_all, x_test_all = dh.data_helper_bert(
                x_train_all, x_val_all, x_test_all, model_select
            )
            # print(x_test_all[0][0])
            x_train_input_ids, x_train_seg_ids, x_train_atten_masks, y_train, \
            x_train_len, trainloader = dh.data_loader(
                x_train_all, batch_size, 'train'
            )
            x_val_input_ids, x_val_seg_ids, x_val_atten_masks, y_val, \
            x_val_len, valloader = dh.data_loader(
                x_val_all, batch_size, 'val'
            )                            
            x_test_input_ids, x_test_seg_ids, x_test_atten_masks, y_test, \
            x_test_len, testloader = dh.data_loader(
                x_test_all, batch_size, 'test'
            )
            ...
            for epoch in range(0, total_epoch):
                ...
                acc, f1_average, precision, recall = model_eval.compute_f1(
                    pred1, y_val
                )
                ...
                with torch.no_grad():
                    ...
                    for ind in range(len(y_test_list)):
                        ...
                        acc, f1_average, precision, recall = model_eval.compute_f1(
                            pred1, y_test_list[ind]
                        )
                        test_f1_average[ind].append(f1_average)
            
            best_epoch = [
                index for index, v in enumerate(val_f1_average) 
                if v == max(val_f1_average)
            ][-1] 
